...
def build_market(market):
    ...
    alpha_path = market_dir / "latest_alpha_model.json"
    if alpha_path.exists():
        model = json.loads(alpha_path.read_text(encoding="utf-8"))
        representative = model.get("representativeParameterSets", [])
        params = representative[0] if representative else {}
        stocks = list((model.get("perStockParameters") or {}).values())

        def enrich(stock):
            improvement = stock.get("improvement", {})
            return {
                **stock,
                "passAll": all(safe_float(improvement.get(key)) > 0 for key in PASS_METRICS),
            }

        enriched = [enrich(stock) for stock in stocks]

        def top_by(key_fn):
            return sorted(enriched, key=key_fn, reverse=True)[:20]
        ...
        evidence = model.get("alphaEvidence", {})
        return {
            "market": market,
            "modelPath": str(alpha_path),
            "modelObjective": model.get("objective", "个股最优/收益增强模型"),
            "parameters": {
                "mode": params.get("mode"),
                "modeLabel": MODE_LABELS.get(params.get("mode"), params.get("mode") or "逐股最优"),
            ...
            "validationGate": {"status": "alpha", "validationPassedCandidates": len(representative)},
            "validationEvidence": evidence,
            "allEvidence": evidence,
            "selectedStockCount": len(enriched),
            "selectedPassAllRate": evidence.get("allObjectivesImprovedRate", 0),
            "allStocks": all_stocks,
            "topStocks": {
                "balanced": top_by(lambda item: safe_float(item.get("strictObjective", item.get("rankScore")))),
                "score": top_by(lambda item: safe_float(item.get("score"))),
                "return": top_by(lambda item: safe_float(item.get("improvement", {}).get("totalReturn"))),
                "sharpe": top_by(lambda item: safe_float(item.get("improvement", {}).get("sharpe"))),
            },
        }
    ...
    with result_path.open("r", encoding="utf-8") as file:
        for line in file:
            ...
            all_stocks.append({**compact, "strictObjective": balanced_score})
    ...
    return {
        ...
        "allStocks": sorted(all_stocks, key=lambda item: str(item.get("symbol") or "")),
        "topStocks": {
...
def main():
    ...
    if verification_path.exists():
        ...
        verification = {
            "passed": all(safe_float(item.get("alphaEvidence", {}).get("strictPassRate")) >= 0.95 for item in summary.get("models", [])),
            "type": "strict-alpha-summary",
            "objective": summary.get("objective"),
            "markets": [
                {
                    "market": item.get("market"),
                    "strictPassRate": item.get("alphaEvidence", {}).get("strictPassRate"),
                    "strictPlusDrawdownPassRate": item.get("alphaEvidence", {}).get("strictPlusDrawdownPassRate"),
                    "fallbackRate": item.get("alphaEvidence", {}).get("fallbackRate"),
                }
